...
plot(df_wilcox$log2fc, -log10(df_wilcox$pval), xlab = "log2(FC)", ylab = "-log10(pval)", pch = 16, col = "gray", main = "T0-T1", sub = "[Wilcoxon rank-sum test]", xlim = c(-M,M))
abline(h = 1, lty = 2)
abline(v = 0, lty = 2)
points(x = df_wilcox$log2fc[idx_TIC], y = -log10(df_wilcox$pval[idx_TIC]), pch = 16, col = "blue")
legend("topright", legend = c("sel","non-sel"), col = c("blue","gray"), pch = 16)
dev.off()

PLOT_MA <- file.path(OUT, "ma.pdf")
pdf(PLOT_MA, width = 4.5, height = 5)
plot(df_wilcox$log.avg, df_wilcox$log2fc, xlab = "mean(log2(CPM))", ylab = "log2(FC)", pch = 16, col = "gray", main = "T0-T1", ylim = c(-M,M))
abline(h = 0, lty = 2)
points(x = df_wilcox$log.avg[idx_TIC], y = df_wilcox$log2fc[idx_TIC], pch = 16, col = "blue")
legend("bottomright", legend = c("sel","non-sel"), col = c("blue","gray"), pch = 16)
dev.off()
