...
# indexes in cpm table
idx_col_p0 <- match(file_list[file_list[,3] == CTRL,1], colnames(cpm))
idx_col_t <- match(file_list[file_list[,3] == CASE,1], colnames(cpm))

# average cpm (for tumors with no associated p0 vial)
n_p0 <- length(idx_col_p0)
if (n_p0 == 1) {
    cpm_p0_avg <- cpm[,idx_col_p0]
} else {
    sum_p0 <- colSums(cpm[,idx_col_p0])
    cpm_p0_avg <- apply(cpm[,idx_col_p0], 1, function(x) sum(sum_p0)*sum(x/(sum_p0*n_p0^2)))
}

cpm_p0 <- cpm[,idx_col_p0]
tot <- colSums(cpm)
tot_p0_avg <- sum(cpm_p0_avg)
cpm_t <- cpm[,idx_col_t]
tot_t <- colSums(cpm_t)

# indexes in file_list
idx_file_p0 <- match(colnames(cpm)[idx_col_p0], file_list[,1])
idx_file_t <- match(colnames(cpm)[idx_col_t], file_list[,1])

fisher.test.pval <- matrix(1, nrow = nrow(cpm), ncol = ncol(cpm_t))
fisher.test.odds <- matrix(1, nrow = nrow(cpm), ncol = ncol(cpm_t))
for (i in 1:nrow(cpm)) {
    for (j in 1:ncol(cpm_t)) {
        # check if a vial exists
        v_t <- vial[idx_file_t[j]]
        if (v_t %in% vial[idx_file_p0]) {
            idx <- which(vial[idx_file_p0] == v_t)
            idx2 <- idx_col_p0[idx]
            fisher_table <- matrix(c(cpm_t[i,j],cpm[i,idx2],tot_t[j]-cpm_t[i,j],tot[idx2]-cpm[i,idx2]), nrow=2, ncol=2)
        } else {
            fisher_table <- matrix(c(cpm_t[i,j],cpm_p0_avg[i],tot_t[j]-cpm_t[i,j],tot_p0_avg-cpm_p0_avg[i]), nrow=2, ncol=2)
        }
