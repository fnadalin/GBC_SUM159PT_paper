...
if (n_p0 == 1) {
    ...
    cpm_p0_avg <- apply(cpm[,idx_col_p0], 1, function(x) round(sum(sum_p0)*sum(x/(sum_p0*n_p0^2))))
}

cpm_t <- round(cpm[,idx_col_t])
tot_p0 <- sum(cpm_p0_avg)
tot_t <- colSums(cpm_t)
...
for (i in 1:nrow(cpm)) {
    for (j in 1:ncol(fisher.test.pval)) {
        fisher_table <- matrix(c(cpm_t[i,j],cpm_p0_avg[i],tot_t[j]-cpm_t[i,j],tot_p0-cpm_p0_avg[i]), nrow=2, ncol=2)
...
v <- apply(cpm[,idx_col_p0], 1, mean) + EPSILON
w <- apply(cpm[,idx_col_t], 1, mean) + EPSILON

PLOT_MA <- file.path(OUT, "maplot.pdf")
pdf(PLOT_MA, width = 5, height = 5)
plot(log2(w*v)/2, log2(w/v))
points(log2(w*v)[is.TIC]/2, log2(w/v)[is.TIC], col = "red")
dev.off()
