...
# compute AUC for each CNV and for each cluster
    
cl <- sort(unique(clusters))
auc <- matrix(0.5, nrow = nrow(M_cnv_cov), ncol = length(cl))
for (h in 1:nrow(M_cnv_cov)) {
    for (k in 1:length(cl)) {
        df <- data.frame(a = M_cnv_cov[h,], b = as.numeric(clusters == cl[k]))
        if (min(df$a) < max(df$a)) {
            rocres <- roc(data = df, response = "b", predictor = "a")
            auc[h,k] <- rocres$auc
        }
    }
}

rownames(auc) <- rownames(M_cnv_cov)
colnames(auc) <- cl
  
write.table(auc, file = file.path(OUTDIR,"auc.txt"), sep = "\t", quote = FALSE)
