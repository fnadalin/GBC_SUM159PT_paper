...
echo "sample.name	readcount	expr.GBC.num	expr.GBC.list" > ${METADATA}
IFS=$'\n'
for line in $(cat ${MATRIX_LIST})
do
    S=$(echo $line | cut -f 1)
    C=$(echo $line | cut -f 2)/"barcodes.tsv.gz"
    CB_CLASS_TSV="${IN_DIR}/$S/classification/CB_classification.tsv"
    
    # extract the CB that are selected also by cellranger-arc
    Rscript ${DIR}/subset_metadata.R ${CB_CLASS_TSV} "$C" ${OUT_DIR}/tmp.tsv

    grep -v "UMIcount" ${OUT_DIR}/tmp.tsv | \
    awk -F "	" -v s=$S -v t="TRUE" -v u="uninfected" -v d="doublet" \
        '{ if ($4 == t && $8 != u && $8 != d) print s "-" $1 "	" s "	" $3 "	" $5 "	" $6}' \
        >> ${METADATA}
