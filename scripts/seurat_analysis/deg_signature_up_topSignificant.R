...
if (length(args) < 4) {
    cat("\nUsage: <min_log2fc> <topGenes> <out> <dea1> [<dea2> <dea3> ... <deaN>] \n")
    cat("\n<min_log2fc> log fold-change cutoff\n")
    cat("<topGenes>   top signifcant genes\n")
...
MIN_LOG2FC <- as.numeric(args[1])
TOP_GENES <- as.numeric(args[2])
SIGNATURE <- args[3]
...
for (i in 4:length(args)) {
    ...
    if (n > TOP_GENES) {
        n <- TOP_GENES
        if (sum(deg_table$p_val_adj == 0) > n) {
            n <- sum(deg_table$p_val_adj == 0)
        } 
    }
    deg_table <- deg_table[1:n,]
    idx <- which(deg_table$avg_log2FC > MIN_LOG2FC)
