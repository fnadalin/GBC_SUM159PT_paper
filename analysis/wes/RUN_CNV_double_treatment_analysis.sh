...
WES_EXP_FILE="in_double_treatment/file.bed"
...
do
    ...
    ID="cnv_double_treatment_atac_${SC_EXP_NAME_ARC[$i]}"
    ARGS=( "${WES_EXP_FILE}" "${SC_EXP_NAME_ARC[$i]}" "${SC_EXP_DIR[$i]}" "${CL_INFO[$i]}" )
    COMMAND="bash ${SCRIPT_DIR}/granges_comparison_scATAC_double.sh ${ARGS[@]}"
    bsub -J ${ID} -o ${ID}.STDOUT -e ${ID}.STDERR ${COMMAND}
    
    ID="cnv_double_treatment_rna_${SC_EXP_NAME_GEX[$i]}"
    ARGS=( "${WES_EXP_FILE}" "${SC_EXP_NAME_GEX[$i]}" "${SC_EXP_OBJ[$i]}" "${SC_SAMPLES[$i]}" "${SC_EXP_DIR[$i]}/features.tsv.gz" \
           "${LIN1_MARKERS}" "${LIN2_MARKERS}" )
    COMMAND="bash ${SCRIPT_DIR}/granges_comparison_scRNA_double.sh ${ARGS[@]}"
    bsub -M ${MEM} -J ${ID} -o ${ID}.STDOUT -e ${ID}.STDERR ${COMMAND}
